...
class LogisticPooled(BaseLogisticModel):
    ...
    def setup_model(self):
        ...
        with self.model:
            ...
            psi_mean = pm.Normal('𝜓_mean', mu=0., sigma=10., shape=1)
            psi_std = pm.HalfNormal('𝜓_std', 5., shape=1)
            psi_spp = pm.Normal(
                '𝜓', mu=psi_mean, sigma=psi_std, shape=self.tree.ntips)
...
if __name__ == "__main__":
    ...
    MOD = LogisticPooled(DATA.tree, DATA.spdata, SUB, model="linear")
    print(MOD.model)
    MOD.sample()
